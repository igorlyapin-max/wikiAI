...
Профиль также задает поведение ответа для MediaWiki, External API и MCP: optional override `llmModel`, `llmTemperature`, `llmMaxTokens`, `llmTimeoutMs`, `systemPrompt`, `conflictSystemPrompt`, `showSources` и `assistantUiMode`. Если поле не задано, Gateway использует fallback из вкладки `LLM` для prompt выдачи и fallback из `Модель доверия` для prompt проверки противоречий.
...
- `retrievalTopK` - сколько кандидатов собрать после retrieval/ACL/trust/rerank. Для `/api/search` это размер поисковой выдачи.
- `contextTopK` - сколько верхних кандидатов из этой выдачи положить в LLM prompt. Для встроенного чата эти chunks сначала схлопываются в source groups по странице/типу/filename, чтобы два фрагмента одного файла не стали двумя разными источниками.
...
- `systemPrompt` - опциональный prompt выдачи для чат-ответа с этим профилем. Пустое поле означает fallback из вкладки `LLM`.
- `conflictSystemPrompt` - опциональный prompt LLM-проверки противоречий для этого профиля. Пустое поле означает fallback из вкладки `Модель доверия`.

Если retrieval нашел 4 кандидата, а `contextTopK=2`, встроенный чат передает модели первые 2 chunks, схлопывает дубли одной страницы/файла в source groups и показывает пользователю только источники, на которые ответ сослался через `[Источник N]`. Если модель не вернула citation markers, Gateway временно показывает весь deduped context как fallback и диагностирует `sourceDisplayMode=no_citations_fallback`. Источник не показывается в чате, если соответствующий chunk не попал в prompt context. Для `/api/search` и External API/MCP явный `topK` в запросе остается request override в пределах `maxTopK`; без него используется `retrievalTopK` выбранного профиля.
...
- RAG `chunkingPolicy`: source-aware rules for `wiki_page`,
  `attachment_text`, `attachment_metadata` and `cmdb_dynamic_snapshot`;
...
Indexing profile still keeps legacy `chunkSize/chunkOverlap/chunkSeparators` as
fallback defaults, but the effective split is chosen by `chunkingPolicy` for
each source type.

### Как выбирать размер chunks

`chunkSize` и `chunkOverlap` измеряются в символах, а не в токенах LLM.
`chunkSize` задает верхнюю границу размера фрагмента: фактический chunk может
быть меньше, потому что Syncer пытается резать текст по структурным
разделителям в таком порядке: заголовки `##`/`###`, пустая строка, строка,
предложение, пробел. Разрез по разделителю применяется только если он найден во
второй половине окна, поэтому слишком маленький `chunkSize` чаще режет текст
механически. `chunkOverlap` повторяет хвост предыдущего chunk в начале
следующего и должен быть меньше `chunkSize`.

Практический baseline для корпоративной wiki:

- `wiki_page`: `chunkSize=800`, `chunkOverlap=120` - основной default для
  регламентов, инструкций, FAQ и смешанной wiki-базы.
- `attachment_text`: `chunkSize=1200`, `chunkOverlap=180` - распознанные
  `pptx/pdf/docx/xlsx`, где chunk должен сохранить контекст слайда, раздела,
  имени файла и родительской страницы.
- `attachment_metadata`: `chunkSize=512`, `chunkOverlap=0` - один компактный
  chunk с filename, MIME, размером, режимом обработки и родительской страницей.
- `cmdb_dynamic_snapshot`: `chunkSize=900`, `chunkOverlap=120` - anonymous
  snapshot динамического блока, который индексируется как отдельный источник
  под ACL родительской MediaWiki-страницы.
- `namespaceOverrides` применяются только к `wiki_page`: например namespace
  `3030` можно резать мельче, если это короткие ИТ-инструкции, не меняя
  размеры вложений на тех же страницах.

Размер chunk нужно выбирать как смысловую единицу ответа. Хороший chunk читается
как самостоятельный источник: в нем есть тема, условие применения и достаточно
контекста, чтобы LLM могла процитировать его без догадок. Слишком маленькие
chunks дают точные совпадения по словам, но теряют заголовок, filename,
родительскую страницу или условие. Слишком большие chunks смешивают разные темы,
размывают embeddings, добавляют шум в BM25/OpenSearch/ColBERT и быстрее
заполняют prompt context.

Overlap держите примерно в диапазоне `10-20%` от `chunkSize`. Значения выше
`25%` обычно не улучшают качество, а только дублируют текст, увеличивают число
chunks, embedding calls, размер BM25/OpenSearch/ColBERT индексов и стоимость
переиндексации. Большой overlap не заменяет правильный `chunkSize`; он нужен
только для сохранения смысла на границах соседних фрагментов.

После изменения legacy `chunkSize/chunkOverlap/chunkSeparators`,
`chunkingPolicy` или `namespaceOverrides` нужен reindex профиля, иначе старые
embeddings и lexical indexes останутся построены по старой нарезке. Проверяйте
настройку через `Debug цепочки`: задайте точный запрос, смысловой вопрос и
запрос по имени файла/родительской странице для вложений.
Настройка считается удачной, если `context chunks` читаются как самостоятельные
источники, правильный документ появляется до LLM-вызова, а ответ ссылается на
ожидаемые `[Источник N]`.
...
сравнивает три слоя: BM25/PostgreSQL chunks, OpenSearch documents и Qdrant
dense payload. Если BM25/PostgreSQL показывает chunks, а OpenSearch или Qdrant
показывают `0`, документ уже распознан, но не был записан в соответствующий
retrieval-слой; исправлять нужно reindex/profile targets, а не поисковый
запрос.
...
- `colbertTimeoutMs`: timeout HTTP-запроса к ColBERT. Глобальный дефолт `5000`; production-профиль `opensearch_hybrid_colbert` использует `12000`, потому CPU rerank на большом наборе кандидатов может занимать около 5 секунд и не должен падать из-за пограничного timeout.
...
Для metadata-only вложений Syncer создает searchable chunk с именем файла, MIME, родительской страницей, размером и `processing mode`. Для распознанных `docx/xlsx/pptx/odt/ods/odp/pdf/txt/image OCR` Syncer добавляет к каждому attachment chunk компактный контекст `Файл`, `MIME` и `Родительская страница`, а затем пишет этот текст в Qdrant embeddings, BM25/search-index, OpenSearch и ColBERT. Поэтому запрос по точному имени файла или по названию родительской страницы становится нормальным retrieval-сигналом, а не отдельным UI-костылем. После включения этой логики старые вложения нужно переиндексировать с `attachmentsEnabled=true`, иначе уже существующие dense-векторы останутся построены по старому тексту без filename context. Ошибки обработки вложений не останавливают reindex страницы: они попадают в `attachmentsFailed` и metadata `error`.

Для `image OCR` Syncer работает offline-first: `TESSERACT_ALLOW_NETWORK_LANG_DOWNLOAD=false` по умолчанию, а локальные `*.traineddata` берутся из `TESSERACT_LANG_PATH`. Если language data отсутствует, изображение все равно индексируется как metadata-only chunk с `error=ocr_language_data_missing` или `error=ocr_language_path_not_configured`; это сохраняет готовность attachment-профиля, но не дает OCR-текста до установки traineddata.
...
В источниках чата вложение отображается отдельно от страницы размещения: `displayTitle` содержит имя файла, `parentPageTitle` и `parentPageUrl` указывают на MediaWiki-страницу, где файл размещен. Inline-ссылка `[Источник N]` для вложения ведет на `parentPageUrl`; direct `attachmentUrl` показывается отдельной ссылкой только если он структурно доступен. Поле `citationIndex` хранит исходный номер источника в prompt, поэтому после фильтрации по citation markers UI может показать, например, только `[5]`, не перенумеровывая его в `[1]`.
...
5. `OpenSearch`: `attachmentDocumentCount > 0`, а проверка filename находит нужный файл; в admin debug trace для этого filename также должен быть `qdrant.found=true`.
...
Для доверенного LAN-demo, где MediaWiki намеренно разрешает anonymous edit,
можно включить `MW_ALLOW_ANON_EDIT=true`: это позволит Syncer записывать
управляемые блоки WikiAI без сервисного пользователя. За пределами демо-стенда
оставляйте значение `false`.
...
Профили поиска выбирают chat profile через `chatProfileId`. Поэтому настройка `retrievalTopK/contextTopK/contextMaxChars`, prompt выдачи и prompt проверки противоречий остаются в `Профили поиска`, а логика "использовать ли прошлые вопросы" теперь живет в `Управление чатами`.
...
- `runMode=risk_only` - режим по умолчанию: проверка запускается только при признаках ненадежности источников в RAG-контексте. Здесь "риск" не означает бизнес- или security-риск; это риск получить ответ на базе спорных или слабо размеченных источников. Сейчас риск считается так:
  ...
  - или в контекст одновременно попали распознанное вложение и обычный текст его родительской wiki-страницы при `attachmentParentConflictMode=risk_only`.
...
- `attachmentParentConflictMode` - отдельный режим для пары "вложение vs родительская страница". Дефолт `risk_only`: если attachment chunk и page chunk той же страницы уже есть в prompt context, это считается risk-сигналом и LLM-анализатор сравнивает их в рамках обычной проверки противоречий. `always` делает то же при каждой найденной паре, `disabled` отключает этот специальный сигнал. Gateway не подтягивает родительскую страницу специально: если она не попала в `contextTopK`, проверка пары пропускается и это видно в `Debug цепочки`.
- `model` - модель LiteLLM/OpenAI-compatible для анализа противоречий. Обычно можно оставить ту же модель, что у LLM-ответа.
- `systemPrompt` - fallback-инструкция, которая отправляется LLM-анализатору как system message, если выбранный retrieval profile не задал `conflictSystemPrompt`. В ней фиксируется правило: сравнивать только предоставленные wiki-источники, не добавлять внешние знания и возвращать JSON. Вопрос пользователя и тексты источников Gateway добавляет отдельным user message во время проверки.
...
Для вложений конфликтом считается только несовместимое утверждение об одном и том же факте: разные даты, статусы, регламенты, числа или взаимоисключающие правила в файле и на странице размещения. Не считается конфликтом то, что файл подробнее страницы, страница короче файла, файл содержит другой раздел темы или приложение просто добавляет контекст. В `Debug цепочки` блок `Attachment vs parent page` показывает найденные пары, вложения без parent page в текущем контексте и причину пропуска.
...
Для автоматической классификации новых страниц основной режим - отдельный WikiAI managed block:
...
Маркеры `WikiAI:semantic:start/end` являются границей владения WikiAI. При следующих webhook система заменяет только содержимое между этими маркерами. Пользовательские SMW-факты и шаблоны вне блока считаются ручными и не перезаписываются. Если включен `skipIfUserFactExists`, свойство не добавляется в managed block, когда такое же прямое SMW-свойство уже найдено вне блока.
...
По умолчанию autofill включен в режиме `apply_empty` с target `managed_block`, чтобы новые страницы могли автоматически получать управляемый блок `WikiAI Semantic` после сохранения. Gateway анализирует страницу по ontology registry, строит рекомендации и сохраняет состояние владения в таблице `ai_smw_autofill_fields`. Runtime-запись в MediaWiki делает только Syncer через сервисного пользователя MediaWiki; для production используйте `MW_SERVICE_USERNAME` и `MW_SERVICE_PASSWORD` или `MW_SERVICE_PASSWORD_SECRET` с Indeed PAM/AAPM.

В админке управление доступно в настройках LLM: включение, режим, target записи, порог уверенности, шаблоны, параметры managed block, namespaces, лимит текста страницы и таблица последних состояний полей.

Target записи:

- `managed_block` - основной режим для новых страниц. Gateway возвращает patch для блока `<!-- WikiAI:semantic:start ... --> ... <!-- WikiAI:semantic:end -->`; Syncer вставляет блок в конец страницы или заменяет существующий блок без изменения пользовательского wikitext вне маркеров.
- `template_params` - совместимый режим для существующих страниц с заранее добавленным шаблоном, например `{{Корпоративный документ}}`. Syncer заполняет только пустые auto-managed параметры этого шаблона.

Доступные режимы:

- `suggest_only` - Gateway сохраняет предложения и диагностику, но Syncer не пишет в wiki.
- `apply_empty` - Syncer вставляет или обновляет managed block либо заполняет пустые auto-managed параметры шаблона, если confidence не ниже `minConfidence`.
