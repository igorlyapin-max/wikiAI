...
### Retrieval profiles для External API и MCP
...
Если `retrievalProfileId` не передан, Gateway сохраняет legacy-поведение и использует текущий global RAG config. Это нужно для обратной совместимости старых клиентов. Если профиль передан явно и он `not_ready`, Gateway возвращает `409 retrieval_profile_not_ready`, без молчаливого fallback.
...
- `current_hybrid` - текущий стек: Qdrant dense + SQLite/Postgres BM25/trigram;
...
## RAG / Embeddings, BM25, ColBERT и суперпозиция поиска
...
- `OpenSearch` - отдельный lexical backend с language analyzer, fuzzy query, highlights, title/text boosts и preview запроса.
- `ColBERT` - URL сервиса, active model/collection, health, candidate indexes и build/promote/cancel.
- `Суперпозиция поиска` - сценарий выдачи: semantic facts, вложения, BM25 и режим ColBERT.
...
- `lexicalBackend`: `sqlite_fts` использует текущий SQLite/Postgres BM25/trigram backend; `opensearch` использует OpenSearch relevance layer. Переключение не строит индекс автоматически: включите target `opensearch` в indexing profile и запустите reindex/backfill.
...
OpenSearch URL в админке относится к Gateway runtime, а не к браузеру администратора. Для `docker-compose` используйте `http://opensearch:9200`: это имя сервиса внутри Docker network. Адрес `http://127.0.0.1:9200` подходит только для ручной проверки с машины, но из контейнера Gateway он укажет на сам контейнер Gateway, а не на OpenSearch. Если OpenSearch не развернут, оставьте `Enable OpenSearch=false`, используйте `sqlite_fts`/BM25 profiles и не выбирайте `opensearch_hybrid*` retrieval profiles до готового URL, индекса и reindex target `opensearch`.
...
По умолчанию live gate проверяет только безопасные сценарии: Gateway `/live`/`/ready`/`/health`/`/metrics`, Syncer `/live`/`/ready`/`/health`/`/metrics`, anonymous MediaWiki API, ResourceLoader bundles `ext.aiadmin`/`ext.aiassistant` и временную Qdrant collection с cleanup. Если задан `MW_TEST_COOKIE` или `WIKIAI_ADMIN_COOKIE`, дополнительно проверяется authenticated `Special:AIAdmin`, наличие вкладок `OpenSearch`/`BM25`/`ColBERT`/`Суперпозиция поиска` и admin endpoints Gateway.
