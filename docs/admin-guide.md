...
Для External API и MCP, если `retrievalProfileId` не передан, Gateway использует `defaultRetrievalProfileId` из вкладки `Внешний API`, когда он задан; пустой внешний default оставляет отдельный compatibility path для старых клиентов. Для встроенного MediaWiki поиска и чата профиль всегда берется из вкладки `Выбор профиля для MediaWiki`, а `retrievalProfileId` из body игнорируется. Если выбранный профиль `not_ready`, Gateway возвращает `409 retrieval_profile_not_ready`, без молчаливого fallback.

### Выбор профиля для MediaWiki

Вкладка `Выбор профиля для MediaWiki` задает единый retrieval profile для встроенного поиска и чата (`/api/search`, `/api/chat`). Default после чистого развертывания - `opensearch_hybrid_colbert`. Это строгий production default: если OpenSearch, ColBERT или нужные индексы не готовы, пользователь получает readiness-ошибку, а не скрытый откат на global RAG config.
...
- `Выбор профиля для MediaWiki` - выбранный retrieval profile для встроенного поиска и чата; semantic facts, вложения, lexical backend и ColBERT управляются через `Профили поиска`.
...
По умолчанию live gate проверяет только безопасные сценарии: Gateway `/live`/`/ready`/`/health`/`/metrics`, Syncer `/live`/`/ready`/`/health`/`/metrics`, anonymous MediaWiki API, ResourceLoader bundles `ext.aiadmin`/`ext.aiassistant`, регистрацию OpenSearch и MediaWiki profile admin routes без cookie и временную Qdrant collection с cleanup. Для admin routes без cookie ожидается `401`, а не `404`: ошибка `Route POST:/api/admin/opensearch/analyze not found` или `Route GET:/api/admin/mediawiki-profile/config not found` означает stale Gateway build/container. Если задан `MW_TEST_COOKIE` или `WIKIAI_ADMIN_COOKIE`, дополнительно проверяется authenticated `Special:AIAdmin`, наличие вкладок `OpenSearch`/`BM25`/`ColBERT`/`Выбор профиля для MediaWiki` и admin endpoints Gateway.
...
6. `Профили поиска` и `Выбор профиля для MediaWiki`: выбранный retrieval profile имеет `includeAttachments=true`; для OpenSearch readiness не должен сообщать `opensearch_attachments`.
