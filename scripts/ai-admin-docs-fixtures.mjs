...
|-
| <code>attachmentParentConflictMode</code> || <code>risk_only</code> || Отдельный сигнал для пары "вложение vs родительская страница". Если распознанное вложение и обычный текст страницы размещения уже попали в контекст ответа, анализатор сравнит их в рамках обычной проверки. Родительская страница не подтягивается отдельно, поэтому <code>topK</code> и стоимость запроса не меняются.
|-
...
Для вложений конфликтом считается только несовместимое утверждение об одном и том же факте: разные даты, статусы, регламенты, числа или взаимоисключающие правила в файле и на странице размещения. Не считается конфликтом то, что файл подробнее страницы, страница короче файла, файл содержит другой раздел темы или просто дополняет контекст.

В <code>Debug цепочки</code> блок <code>Attachment vs parent page</code> показывает найденные пары, вложения без parent page в текущем контексте и причину пропуска. Если файл есть, но страницы нет среди context sources, проверка пары не запускается.
